...
namespace Core.Input
{
    ...
    public class KeyboardMouseInput : CameraInputScheme
    {
        ...
        {
            Debug.Log("OnEnable called for KeyboardMouseInput script");
            if (!InputController.instanceExists)
            {
                Debug.LogError("[UI] Keyboard and Mouse UI requires InputController");
                return;
            }
            Debug.Log("InputController instance exists");
        ...
        protected virtual void OnDrag(PointerActionInfo pointer)
        {
            ...
            if ((mouseInfo != null) && (mouseInfo.mouseButtonId == 2) && (mouseInfo.isDragging))
            {
                Debug.Log("Middle mouse button dragged.");
                DoMiddleMousePan(mouseInfo);
            }
            else if (mouseInfo != null && mouseInfo.isDragging)
            {
                Debug.Log("Mouse button " + mouseInfo.mouseButtonId + " dragged.");
            }
            if (cameraRig != null)
            {
        ...
        protected virtual void OnPress(PointerActionInfo pointer)
        {
            ...
            if ((mouseInfo != null) && (mouseInfo.mouseButtonId == 2))
            {
                ...
                mouseInfo.startDragPosition = mouseInfo.currentPosition;
            }
            if (cameraRig != null)
            {
                DoMiddleMousePan(mouseInfo);
        ...
        protected void DoMiddleMousePan(MouseButtonInfo mouseInfo)
        {
            if (mouseInfo.isDragging)
            {
                ...
                Vector2 panVector = mouseInfo.currentPosition - mouseInfo.startDragPosition;
                panVector = (panVector * Time.deltaTime * mouseRmbPanSpeed * zoomRatio) / screenPanThreshold;
